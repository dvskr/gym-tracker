...
class HealthService {
 ...
 logger.error('R Check permissions failed:', error);
  ...
  async readHeartRate(
    ...
    try {
      ...
      return data.map((record: HeartRateRecord) => ({
 ...
 logger.error('R Failed to read steps:', error);
  ...
  async readSleep(
    ...
    try {
      ...
      return data.map((record: SleepRecord) => ({
 ...
 logger.log(` Estimated calories: ${estimated} kcal (${durationMinutes}min @ ${rate}cal/min)`
    );
    return estimated;
  }
 ...
 logger.warn('a No health permissions to read weight');
    ...
    try {
 logger.log(` Reading weight history from ${startDate.toLocaleDateString()} to ${endDate.toLocaleDateString()}`
      );
 ...
 logger.log(`S& Retrieved ${records.length} weight records from health platform`);
      ...
      let bodyFatRecords: BodyFatRecord[] = [];
      try {
 ...
 logger.log(`Retrieved ${records.length} heart rate records`);
      ...
      interface HeartRateRecordWithSamples {
        beatsPerMinute?: number;
      ...
      records.forEach((record: HeartRateRecordWithSamples) => {
 ...
 logger.log(`Retrieved ${records.length} step records`);
 ...
 logger.warn('a No health permissions to read sleep');
    ...
    try {
 logger.log('x Reading last night\'s sleep data...');
 ...
 logger.log(`Retrieved ${records.length} sleep session(s)`);
 ...
 logger.error('R Failed to write weight:', error);
  ...
  async writeBodyFat(percentage: number, time: Date): Promise<boolean> {
    try {
 logger.log(`Writing body fat to health platform: ${percentage}%`);
