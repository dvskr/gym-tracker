...
export const buildCoachContextLegacy = async (userId: string): Promise<string> => {
  try {
    ...
    if (injuries.data && injuries.data.length > 0) {
      context += `\n�a���� ACTIVE INJURIES (CRITICAL - DO NOT suggest exercises that aggravate these):\n`;
      injuries.data.forEach((injury: any) => {
        ...
        if (injury.avoid_movements && injury.avoid_movements.length > 0) {
          context += `  �R Avoid: ${injury.avoid_movements.join(', ')}\n`;
        }
        
        if (injury.avoid_exercises && injury.avoid_exercises.length > 0) {
          context += `  �R Avoid exercises: ${injury.avoid_exercises.join(', ')}\n`;
        }
      });
    }
      ...
      mainLiftHistory.forEach((lift: any) => {
        if (lift.sessions.length > 0) {
          ...
          if (lift.trend === 'plateau') {
            context += ` �a���� PLATEAU (no progress in ${lift.weeksSinceImprovement} weeks)`;
    ...
    if (todayCheckin.data) {
      ...
      if (c.sleep_quality && c.sleep_quality <= 2) {
        context += '\n�a���� Note: Poor sleep - consider lighter training\n';
      }
      if (c.energy_level && c.energy_level <= 2) {
        context += '�a���� Note: Low energy - may need active recovery\n';
      }
      if (c.soreness_level && c.soreness_level >= 4) {
        context += '�a���� Note: High soreness - focus on unaffected muscle groups\n';
      }
    }
    ...
    context += 'GOOD: "Your last bench was 185×8. Try 190×6 or go for 185×10"\n\n';
...
export const buildInjuryContext = async (userId: string): Promise<string> => {
  try {
    ...
    let context = '\n�a���� IMPORTANT - Active Injuries/Limitations:\n';
    ...
    for (const injury of data) {
      ...
      if (injury.avoid_movements && injury.avoid_movements.length > 0) {
        context += `  �R Avoid movements: ${injury.avoid_movements.join(', ')}\n`;
      }
...
export const buildCoachContext = async (userId: string): Promise<CoachContext> => {
  try {
    ...
    if (dataState.isNewUser) {
      contextText += `
�a�����a�️�a���� NEW USER - ZERO WORKOUT DATA �a�����a�️�a����
...
�xa��xa��xa� ACTIVE INJURIES - CRITICAL �xa��xa��xa�

`;
      for (const injury of injuries) {
        contextText += `
�x� ${injury.bodyPart.toUpperCase()} - ${injury.type || 'Injury'} (Severity: ${injury.severity})
   �R NEVER suggest: ${(injury.avoidExercises || []).join(', ') || 'N/A'}
   �R NEVER suggest movements: ${(injury.avoidMovements || []).join(', ') || 'N/A'}
...
`;
    ...
    if (profile) {
      ...
      if (profile.available_equipment && profile.available_equipment.length > 0) {
        ...
        contextText += `  �a���� ONLY suggest exercises using this equipment!\n`;
      } else {
    ...
    if (dataState.hasCheckin && checkin) {
      ...
      if (checkin.soreness && checkin.soreness.length > 0) {
        ...
        contextText += `  �a���� Avoid overworking these areas today\n`;
      }
...
- Workout History: ${dataState.hasWorkouts ? `�S& Yes (${dataState.workoutCount} recent)` : '�R None'}
- Personal Records: ${dataState.hasPRs ? '�S& Yes' : '�R None'}
- Active Injuries: ${dataState.hasInjuries ? '�xa� Yes (see above)' : '�S& None'}
- Today's Check-in: ${dataState.hasCheckin ? '�S& Yes (see above)' : '�R None'}
- Equipment Info: ${dataState.hasEquipment ? '�S& Yes (see above)' : '�R Not specified'}
- Goals Set: ${dataState.hasGoals ? '�S& Yes' : '�R Not specified'}

�a���� IMPORTANT: Base your responses ONLY on the data marked with �S& above.
...
`;
    ...
    return {
      text: '�a���� Error loading user data. Provide general guidance only.',
      flags: {
        hasWorkouts: false,
        hasPRs: false,
        hasInjuries: false,
...
function buildWorkoutHistoryContext(workouts: any[]): string {
  ...
  context += `�x` Workout Frequency: ${frequency} times/week (${workouts.length} workouts in ${daysSinceFirst} days)\n`;
...
function buildPRContext(prs: any[]): string {
  ...
  let context = '�x�  PERSONAL RECORDS:\n';
