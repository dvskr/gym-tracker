...
// Strong's complete exercise list (from screenshots)
const strongExercises = [
  ...
  "V Up",
  "Walking", "Wide Pull Up", "Wrist Roller",
  "Yoga",
  "Zercher Squat (Barbell)"
];

// Load our exercises
const ourExercises = JSON.parse(
  fs.readFileSync('scripts/our-exercises-list.json', 'utf-8')
);
...
const ourNormalized = ourExercises.map((ex: any) => ({
  original: ex.name,
...
for (const strongEx of strongExercises) {
  const strongNorm = normalize(strongEx);
  const match = ourNormalized.find((our: any) => 
    our.normalized.includes(strongNorm.slice(0, 15)) || 
    strongNorm.includes(our.normalized.slice(0, 15))
  );
  
  if (match) {
    results.found.push(strongEx);
    results.matchDetails.push({
      strong: strongEx,
      ours: match.original
    });
  } else {
    results.missing.push(strongEx);
  }
}
...
console.log('-'.repeat(60));
results.missing.forEach((ex, i) => console.log(`${i + 1}. ${ex}`));
